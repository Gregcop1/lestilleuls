...
class DoctrineCommandController extends DoctrineCommandController_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	 ...
	 protected function emitAfterDatabaseMigration() {
		...
		$result = parent::emitAfterDatabaseMigration();
			...
			try {
				...
				$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Flow\Command\DoctrineCommandController', 'emitAfterDatabaseMigration', $methodArguments);
				$result = $this->Flow_Aop_Proxy_invokeJoinPoint($joinPoint);
				$methodArguments = $joinPoint->getMethodArguments();

				if (isset($this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices['emitAfterDatabaseMigration']['TYPO3\Flow\Aop\Advice\AfterReturningAdvice'])) {
					$advices = $this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices['emitAfterDatabaseMigration']['TYPO3\Flow\Aop\Advice\AfterReturningAdvice'];
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Flow\Command\DoctrineCommandController', 'emitAfterDatabaseMigration', $methodArguments, NULL, $result);
	...
	foreach ($allReflectedProperties as $reflectionProperty) {
		...
		if ($reflectionService->isPropertyAnnotatedWith('TYPO3\Flow\Command\DoctrineCommandController', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('TYPO3\Flow\Command\DoctrineCommandController', $propertyName, 'var');
