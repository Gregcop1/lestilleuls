<?php

namespace TYPO3\Flow\Persistence\Doctrine\Proxies\__CG__\TYPO3\Media\Domain\Model;

/**
 * THIS CLASS WAS GENERATED BY THE DOCTRINE ORM. DO NOT EDIT THIS FILE.
 */
class Image extends \TYPO3\Media\Domain\Model\Image implements \Doctrine\ORM\Proxy\Proxy
{
    private $_entityPersister;
    private $_identifier;
    public $__isInitialized__ = false;
    public function __construct($entityPersister, $identifier)
    {
        $this->_entityPersister = $entityPersister;
        $this->_identifier = $identifier;
    }
    /** @private */
    public function __load()
    {
        if (!$this->__isInitialized__ && $this->_entityPersister) {
            $this->__isInitialized__ = true;

            if (method_exists($this, "__wakeup")) {
                // call this after __isInitialized__to avoid infinite recursion
                // but before loading to emulate what ClassMetadata::newInstance()
                // provides.
                $this->__wakeup();
            }

            if ($this->_entityPersister->load($this->_identifier, $this) === null) {
                throw new \Doctrine\ORM\EntityNotFoundException();
            }
            unset($this->_entityPersister, $this->_identifier);
        }
    }

    /** @private */
    public function __isInitialized()
    {
        return $this->__isInitialized__;
    }

    
    public function __wakeup()
    {
        $this->__load();
        return parent::__wakeup();
    }

    public function Flow_Aop_Proxy_fixMethodsAndAdvicesArrayForDoctrineProxies()
    {
        $this->__load();
        return parent::Flow_Aop_Proxy_fixMethodsAndAdvicesArrayForDoctrineProxies();
    }

    public function Flow_Aop_Proxy_fixInjectedPropertiesForDoctrineProxies()
    {
        $this->__load();
        return parent::Flow_Aop_Proxy_fixInjectedPropertiesForDoctrineProxies();
    }

    public function Flow_Aop_Proxy_invokeJoinPoint(\TYPO3\Flow\Aop\JoinPointInterface $joinPoint)
    {
        $this->__load();
        return parent::Flow_Aop_Proxy_invokeJoinPoint($joinPoint);
    }

    public function initializeObject($cause)
    {
        $this->__load();
        return parent::initializeObject($cause);
    }

    public function initializeImageSizeAndType()
    {
        $this->__load();
        return parent::initializeImageSizeAndType();
    }

    public function getWidth()
    {
        $this->__load();
        return parent::getWidth();
    }

    public function getHeight()
    {
        $this->__load();
        return parent::getHeight();
    }

    public function getAspectRatio($respectOrientation = false)
    {
        $this->__load();
        return parent::getAspectRatio($respectOrientation);
    }

    public function getOrientation()
    {
        $this->__load();
        return parent::getOrientation();
    }

    public function isOrientationSquare()
    {
        $this->__load();
        return parent::isOrientationSquare();
    }

    public function isOrientationLandscape()
    {
        $this->__load();
        return parent::isOrientationLandscape();
    }

    public function isOrientationPortrait()
    {
        $this->__load();
        return parent::isOrientationPortrait();
    }

    public function getType()
    {
        $this->__load();
        return parent::getType();
    }

    public function getFileExtension()
    {
        $this->__load();
        return parent::getFileExtension();
    }

    public function getThumbnail($maximumWidth = NULL, $maximumHeight = NULL, $ratioMode = 'inset')
    {
        $this->__load();
        return parent::getThumbnail($maximumWidth, $maximumHeight, $ratioMode);
    }

    public function setImageVariants(array $imageVariants)
    {
        $this->__load();
        return parent::setImageVariants($imageVariants);
    }

    public function getImageVariants()
    {
        $this->__load();
        return parent::getImageVariants();
    }

    public function createImageVariant(array $processingInstructions, $alias = NULL)
    {
        $this->__load();
        return parent::createImageVariant($processingInstructions, $alias);
    }

    public function removeImageVariant(\TYPO3\Media\Domain\Model\ImageVariant $imageVariant)
    {
        $this->__load();
        return parent::removeImageVariant($imageVariant);
    }

    public function getImageVariantByAlias($alias)
    {
        $this->__load();
        return parent::getImageVariantByAlias($alias);
    }

    public function removeImageVariantByAlias($alias)
    {
        $this->__load();
        return parent::removeImageVariantByAlias($alias);
    }

    public function getIdentifier()
    {
        $this->__load();
        return parent::getIdentifier();
    }

    public function getLabel()
    {
        $this->__load();
        return parent::getLabel();
    }

    public function getLastModified()
    {
        $this->__load();
        return parent::getLastModified();
    }

    public function setResource(\TYPO3\Flow\Resource\Resource $resource)
    {
        $this->__load();
        return parent::setResource($resource);
    }

    public function getResource()
    {
        $this->__load();
        return parent::getResource();
    }

    public function setTitle($title)
    {
        $this->__load();
        return parent::setTitle($title);
    }

    public function getTitle()
    {
        $this->__load();
        return parent::getTitle();
    }

    public function setCaption($caption)
    {
        $this->__load();
        return parent::setCaption($caption);
    }

    public function getCaption()
    {
        $this->__load();
        return parent::getCaption();
    }

    public function getTags()
    {
        $this->__load();
        return parent::getTags();
    }

    public function addTag(\TYPO3\Media\Domain\Model\Tag $tag)
    {
        $this->__load();
        return parent::addTag($tag);
    }

    public function setTags(\Doctrine\Common\Collections\Collection $tags)
    {
        $this->__load();
        return parent::setTags($tags);
    }

    public function removeTag(\TYPO3\Media\Domain\Model\Tag $tag)
    {
        $this->__load();
        return parent::removeTag($tag);
    }


    public function __sleep()
    {
        return array_merge(array('__isInitialized__'), parent::__sleep());
    }

    public function __clone()
    {
        if (!$this->__isInitialized__ && $this->_entityPersister) {
            $this->__isInitialized__ = true;
            $class = $this->_entityPersister->getClassMetadata();
            $original = $this->_entityPersister->load($this->_identifier);
            if ($original === null) {
                throw new \Doctrine\ORM\EntityNotFoundException();
            }
            foreach ($class->reflFields as $field => $reflProperty) {
                $reflProperty->setValue($this, $reflProperty->getValue($original));
            }
            unset($this->_entityPersister, $this->_identifier);
        }
        parent::__clone();
    }
}