 ...
 */
class FlowAnnotationDriver_Original implements \Doctrine\Common\Persistence\Mapping\Driver\MappingDriver, \TYPO3\Flow\Aop\Pointcut\PointcutFilterInterface {
 ...
 */
class FlowAnnotationDriver extends FlowAnnotationDriver_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	 ...
	 */
	public function __construct() {
		if (get_class($this) === 'TYPO3\Flow\Persistence\Doctrine\Mapping\Driver\FlowAnnotationDriver') \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->setInstance('TYPO3\Flow\Persistence\Doctrine\Mapping\Driver\FlowAnnotationDriver', $this);
		parent::__construct();
		if ('TYPO3\Flow\Persistence\Doctrine\Mapping\Driver\FlowAnnotationDriver' === get_class($this)) {
	...
	$reflectedClass = new \ReflectionClass('TYPO3\Flow\Persistence\Doctrine\Mapping\Driver\FlowAnnotationDriver');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if (isset($this->Flow_Injected_Properties) && is_array($this->Flow_Injected_Properties) && in_array($propertyName, $this->Flow_Injected_Properties)) continue;
		if ($reflectionService->isPropertyAnnotatedWith('TYPO3\Flow\Persistence\Doctrine\Mapping\Driver\FlowAnnotationDriver', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('TYPO3\Flow\Persistence\Doctrine\Mapping\Driver\FlowAnnotationDriver', $propertyName, 'var');
