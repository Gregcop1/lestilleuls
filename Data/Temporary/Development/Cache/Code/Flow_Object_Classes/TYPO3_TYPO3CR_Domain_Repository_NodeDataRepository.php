 ...
 */
class NodeDataRepository_Original extends Repository {
 ...
 */
class NodeDataRepository extends NodeDataRepository_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		...
		if (get_class($this) === 'TYPO3\TYPO3CR\Domain\Repository\NodeDataRepository') \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->setInstance('TYPO3\TYPO3CR\Domain\Repository\NodeDataRepository', $this);
		parent::__construct();
		if ('TYPO3\TYPO3CR\Domain\Repository\NodeDataRepository' === get_class($this)) {
	 ...
	 protected function emitRepositoryObjectsPersisted() {
		...
		$result = parent::emitRepositoryObjectsPersisted();
			...
			try {
				...
				$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\TYPO3CR\Domain\Repository\NodeDataRepository', 'emitRepositoryObjectsPersisted', $methodArguments);
				$result = $this->Flow_Aop_Proxy_invokeJoinPoint($joinPoint);
				$methodArguments = $joinPoint->getMethodArguments();

				if (isset($this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices['emitRepositoryObjectsPersisted']['TYPO3\Flow\Aop\Advice\AfterReturningAdvice'])) {
					...
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\TYPO3CR\Domain\Repository\NodeDataRepository', 'emitRepositoryObjectsPersisted', $methodArguments, NULL, $result);
	...
	$reflectedClass = new \ReflectionClass('TYPO3\TYPO3CR\Domain\Repository\NodeDataRepository');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if (isset($this->Flow_Injected_Properties) && is_array($this->Flow_Injected_Properties) && in_array($propertyName, $this->Flow_Injected_Properties)) continue;
		if ($reflectionService->isPropertyAnnotatedWith('TYPO3\TYPO3CR\Domain\Repository\NodeDataRepository', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('TYPO3\TYPO3CR\Domain\Repository\NodeDataRepository', $propertyName, 'var');
