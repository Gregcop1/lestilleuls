...
class PersistenceManager extends PersistenceManager_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
		...
		$result = parent::emitAllObjectsPersisted();
			...
			try {
				...
				if (isset($this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices['emitAllObjectsPersisted']['TYPO3\Flow\Aop\Advice\AfterReturningAdvice'])) {
					...
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Flow\Persistence\Generic\PersistenceManager', 'emitAllObjectsPersisted', $methodArguments, NULL, $result);
	...
	foreach ($allReflectedProperties as $reflectionProperty) {
		...
		if ($reflectionService->isPropertyAnnotatedWith('TYPO3\Flow\Persistence\Generic\PersistenceManager', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('TYPO3\Flow\Persistence\Generic\PersistenceManager', $propertyName, 'var');
