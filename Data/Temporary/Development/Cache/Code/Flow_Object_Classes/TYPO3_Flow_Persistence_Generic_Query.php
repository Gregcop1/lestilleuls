...
class Query extends Query_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	 ...
	 */
	 public function execute($cacheResult = FALSE) {
		...
		$result = parent::execute($cacheResult);
			...
			try {
				...
				$methodArguments['cacheResult'] = $cacheResult;
	...
	foreach ($allReflectedProperties as $reflectionProperty) {
		...
		if ($reflectionService->isPropertyAnnotatedWith('TYPO3\Flow\Persistence\Generic\Query', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('TYPO3\Flow\Persistence\Generic\Query', $propertyName, 'var');
