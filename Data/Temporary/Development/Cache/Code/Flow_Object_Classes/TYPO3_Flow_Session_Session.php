...
class Session extends Session_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	 ...
	 public function start() {
		...
		try {
				...
				if (isset($this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices['start']['TYPO3\Flow\Aop\Advice\AfterAdvice'])) {
					...
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Flow\Session\Session', 'start', $methodArguments, NULL, $result);
				...
				if (!$afterAdviceInvoked && isset($this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices['start']['TYPO3\Flow\Aop\Advice\AfterAdvice'])) {
					$advices = $this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices['start']['TYPO3\Flow\Aop\Advice\AfterAdvice'];
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Flow\Session\Session', 'start', $methodArguments, NULL, NULL, $exception);
					foreach ($advices as $advice) {
						$advice->invoke($joinPoint);
					}
				}
	...
	foreach ($allReflectedProperties as $reflectionProperty) {
		...
		if ($reflectionService->isPropertyAnnotatedWith('TYPO3\Flow\Session\Session', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('TYPO3\Flow\Session\Session', $propertyName, 'var');
