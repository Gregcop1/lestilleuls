 ...
 */
class SchemaController extends SchemaController_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		...
		if (get_class($this) === 'TYPO3\Neos\Controller\Backend\SchemaController') \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->setInstance('TYPO3\Neos\Controller\Backend\SchemaController', $this);
		if ('TYPO3\Neos\Controller\Backend\SchemaController' === get_class($this)) {
	 ...
	 public function vieSchemaAction() {
		...
		$result = parent::vieSchemaAction();
			...
			try {
				...
				$adviceChain->rewind();
				$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Neos\Controller\Backend\SchemaController', 'vieSchemaAction', $methodArguments, $adviceChain);
	 ...
	 public function nodeTypeSchemaAction() {
		...
		$result = parent::nodeTypeSchemaAction();
			...
			try {
				...
				$adviceChain = $adviceChains['TYPO3\Flow\Aop\Advice\AroundAdvice'];
				$adviceChain->rewind();
				$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Neos\Controller\Backend\SchemaController', 'nodeTypeSchemaAction', $methodArguments, $adviceChain);
				$result = $adviceChain->proceed($joinPoint);
				$methodArguments = $joinPoint->getMethodArguments();
	...
	$reflectedClass = new \ReflectionClass('TYPO3\Neos\Controller\Backend\SchemaController');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if (isset($this->Flow_Injected_Properties) && is_array($this->Flow_Injected_Properties) && in_array($propertyName, $this->Flow_Injected_Properties)) continue;
		if ($reflectionService->isPropertyAnnotatedWith('TYPO3\Neos\Controller\Backend\SchemaController', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('TYPO3\Neos\Controller\Backend\SchemaController', $propertyName, 'var');
	 ...
	 */
	 private function Flow_Proxy_injectProperties() {
		...
		$vieSchemaBuilder_reference = &$this->vieSchemaBuilder;
		$this->vieSchemaBuilder = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->getInstance('TYPO3\Neos\Service\VieSchemaBuilder');
		if ($this->vieSchemaBuilder === NULL) {
			$this->vieSchemaBuilder = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->getLazyDependencyByHash('8d11bc7a73fc1f52f3e5f59fa8b602dc', $vieSchemaBuilder_reference);
			if ($this->vieSchemaBuilder === NULL) {
				$this->vieSchemaBuilder = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->createLazyDependency('8d11bc7a73fc1f52f3e5f59fa8b602dc',  $vieSchemaBuilder_reference, 'TYPO3\Neos\Service\VieSchemaBuilder', function() { return \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->get('TYPO3\Neos\Service\VieSchemaBuilder'); });
			}
		}
		$nodeTypeSchemaBuilder_reference = &$this->nodeTypeSchemaBuilder;
		$this->nodeTypeSchemaBuilder = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->getInstance('TYPO3\Neos\Service\NodeTypeSchemaBuilder');
		if ($this->nodeTypeSchemaBuilder === NULL) {
			$this->nodeTypeSchemaBuilder = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->getLazyDependencyByHash('2c6771efe1ffef78a39f0585404ab874', $nodeTypeSchemaBuilder_reference);
			if ($this->nodeTypeSchemaBuilder === NULL) {
				$this->nodeTypeSchemaBuilder = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->createLazyDependency('2c6771efe1ffef78a39f0585404ab874',  $nodeTypeSchemaBuilder_reference, 'TYPO3\Neos\Service\NodeTypeSchemaBuilder', function() { return \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->get('TYPO3\Neos\Service\NodeTypeSchemaBuilder'); });
			}
		}
