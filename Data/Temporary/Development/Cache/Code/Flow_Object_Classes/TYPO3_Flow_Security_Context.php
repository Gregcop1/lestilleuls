...
class Context extends Context_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		...
		if (get_class($this) === 'TYPO3\Flow\Security\Context') {
	 ...
	 public function getCsrfProtectionToken() {
		...
		$result = parent::getCsrfProtectionToken();
			...
			try {
				...
				if (isset($this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices['getCsrfProtectionToken']['TYPO3\Flow\Aop\Advice\BeforeAdvice'])) {
					$advices = $this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices['getCsrfProtectionToken']['TYPO3\Flow\Aop\Advice\BeforeAdvice'];
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Flow\Security\Context', 'getCsrfProtectionToken', $methodArguments);
	 ...
	 public function setInterceptedRequest(\TYPO3\Flow\Mvc\ActionRequest $interceptedRequest = NULL) {
		...
		$result = parent::setInterceptedRequest($interceptedRequest);
			...
			try {
				...
				$adviceChain = $adviceChains['TYPO3\Flow\Aop\Advice\AroundAdvice'];
				$adviceChain->rewind();
				$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Flow\Security\Context', 'setInterceptedRequest', $methodArguments, $adviceChain);
				$result = $adviceChain->proceed($joinPoint);
				$methodArguments = $joinPoint->getMethodArguments();
	...
	foreach ($allReflectedProperties as $reflectionProperty) {
		...
		if ($reflectionService->isPropertyAnnotatedWith('TYPO3\Flow\Security\Context', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('TYPO3\Flow\Security\Context', $propertyName, 'var');
	 ...
	 */
	 private function Flow_Proxy_injectProperties() {
		...
		$this->sessionManager = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->getInstance('TYPO3\Flow\Session\SessionManager');
		if ($this->sessionManager === NULL) {
			$this->sessionManager = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->getLazyDependencyByHash('d6fde3e13a932cb1e34ae30af8eb9c9a', $sessionManager_reference);
			if ($this->sessionManager === NULL) {
				$this->sessionManager = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->createLazyDependency('d6fde3e13a932cb1e34ae30af8eb9c9a',  $sessionManager_reference, 'TYPO3\Flow\Session\SessionManager', function() { return \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->get('TYPO3\Flow\Session\SessionManager'); });
			}
		}
