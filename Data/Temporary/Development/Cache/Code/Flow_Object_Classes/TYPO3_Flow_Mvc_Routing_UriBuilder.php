...
class UriBuilder extends UriBuilder_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
		...
		$this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices = array(
			'uriFor' => array(
				'TYPO3\Flow\Aop\Advice\AroundAdvice' => array(
					new \TYPO3\Flow\Aop\Advice\AroundAdvice('TYPO3\Neos\Aspects\PluginUriAspect', 'rewritePluginViewUris', $objectManager, NULL),
				),
			),
		);
	}
	 ...
	 public function uriFor($actionName, $controllerArguments = array(), $controllerName = NULL, $packageKey = NULL, $subPackageKey = NULL) {
		...
		$result = parent::uriFor($actionName, $controllerArguments, $controllerName, $packageKey, $subPackageKey);
			...
			try {
				...
				$adviceChain->rewind();
				$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Flow\Mvc\Routing\UriBuilder', 'uriFor', $methodArguments, $adviceChain);
	...
	foreach ($allReflectedProperties as $reflectionProperty) {
		...
		if ($reflectionService->isPropertyAnnotatedWith('TYPO3\Flow\Mvc\Routing\UriBuilder', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('TYPO3\Flow\Mvc\Routing\UriBuilder', $propertyName, 'var');
	 ...
	 */
	 private function Flow_Proxy_injectProperties() {
		...
		$this->createRelativePaths = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->get('TYPO3\Flow\Configuration\ConfigurationManager')->getConfiguration(\TYPO3\Flow\Configuration\ConfigurationManager::CONFIGURATION_TYPE_SETTINGS, 'TYPO3.Flow.compatibility.uriBuilder.createRelativePaths');
$this->Flow_Injected_Properties = array (
  0 => 'router',
  1 => 'environment',
  2 => 'createRelativePaths',
);
	}
}
#