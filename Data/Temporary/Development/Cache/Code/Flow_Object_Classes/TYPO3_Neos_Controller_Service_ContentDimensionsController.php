 ...
 * REST service controller for managing content dimensions
 */
class ContentDimensionsController extends ContentDimensionsController_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		...
		if ('TYPO3\Neos\Controller\Service\ContentDimensionsController' === get_class($this)) {
	 ...
	 public function indexAction() {
		...
		$result = parent::indexAction();
			...
			try {
				...
				$adviceChain->rewind();
				$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Neos\Controller\Service\ContentDimensionsController', 'indexAction', $methodArguments, $adviceChain);
	 ...
	 protected function errorAction() {
		...
		$result = parent::errorAction();
			...
			try {
				...
				$adviceChain = $adviceChains['TYPO3\Flow\Aop\Advice\AroundAdvice'];
				$adviceChain->rewind();
				$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Neos\Controller\Service\ContentDimensionsController', 'errorAction', $methodArguments, $adviceChain);
				$result = $adviceChain->proceed($joinPoint);
				$methodArguments = $joinPoint->getMethodArguments();
	...
	$reflectedClass = new \ReflectionClass('TYPO3\Neos\Controller\Service\ContentDimensionsController');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if (isset($this->Flow_Injected_Properties) && is_array($this->Flow_Injected_Properties) && in_array($propertyName, $this->Flow_Injected_Properties)) continue;
		if ($reflectionService->isPropertyAnnotatedWith('TYPO3\Neos\Controller\Service\ContentDimensionsController', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('TYPO3\Neos\Controller\Service\ContentDimensionsController', $propertyName, 'var');
...
$this->Flow_Injected_Properties = array (
  0 => 'settings',
  1 => 'contentDimensionPresetSource',
  2 => 'objectManager',
  3 => 'reflectionService',
